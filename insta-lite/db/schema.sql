...
-- PostgreSQL 18
-- Dumped from local database and cleaned up
...
CREATE FUNCTION public.update_post_likes_count() RETURNS trigger
    LANGUAGE plpgsql
    AS $$ BEGIN IF (TG_OP = 'INSERT') THEN UPDATE public.posts SET likes_count = likes_count + 1 WHERE post_id = NEW.post_id; RETURN NEW; ELSIF (TG_OP = 'DELETE') THEN UPDATE public.posts SET likes_count = GREATEST(likes_count - 1, 0) WHERE post_id = OLD.post_id; RETURN OLD; END IF; RETURN NULL; END; $$;

CREATE FUNCTION public.update_post_comments_count() RETURNS trigger
    LANGUAGE plpgsql
    AS $$ BEGIN IF (TG_OP = 'INSERT') THEN UPDATE public.posts SET comments_count = comments_count + 1 WHERE post_id = NEW.post_id; RETURN NEW; ELSIF (TG_OP = 'DELETE') THEN UPDATE public.posts SET comments_count = GREATEST(comments_count - 1, 0) WHERE post_id = OLD.post_id; RETURN OLD; END IF; RETURN NULL; END; $$;

CREATE FUNCTION public.update_comment_likes_count() RETURNS trigger
    LANGUAGE plpgsql
    AS $$ BEGIN IF (TG_OP = 'INSERT') THEN UPDATE public.comments SET likes_count = likes_count + 1 WHERE comment_id = NEW.comment_id; RETURN NEW; ELSIF (TG_OP = 'DELETE') THEN UPDATE public.comments SET likes_count = GREATEST(likes_count - 1, 0) WHERE comment_id = OLD.comment_id; RETURN OLD; END IF; RETURN NULL; END; $$;
