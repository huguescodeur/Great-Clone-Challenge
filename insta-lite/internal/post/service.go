...
type PostService struct {
	...
	db              *sql.DB
	bus             *events.TypedBus[events.PostCreatedEvent]
	likesCounter    LikesCounter
	commentsCounter CommentsCounter
}

func NewServicePost(p PostStore, m postmedia.PostMediaStore, u user.UserStore, db *sql.DB, bus *events.TypedBus[events.PostCreatedEvent], lc LikesCounter, cc CommentsCounter) *PostService {
	return &PostService{postStore: p, postMediaStore: m, userStore: u, db: db, bus: bus, likesCounter: lc, commentsCounter: cc}
}
...
func (s *PostService) CreatePost(ctx context.Context, post Post, medias []postmedia.PostMedia) (*PostResponse, error) {
	...
	if _, err := s.userStore.UpdatePostCount(ctx, tx, insertedPost.UserID, 1); err != nil {
		return nil, err
	}
	...
	s.bus.Publish(events.PostCreatedEvent{
...
func (s *PostService) applyLiveLikesCounts(ctx context.Context, posts []*Post) {
	if len(posts) == 0 {
		return
	}
	...
	if err != nil {
		log.Printf("[post] échec lecture compteurs likes Redis: %v", err)
		return
	}
...
func (s *PostService) applyLiveCommentsCounts(ctx context.Context, posts []*Post) {
	if len(posts) == 0 {
		return
	}
	ids := make([]uuid.UUID, len(posts))
