...
type CommentService struct {
	...
	commentLikesCounter CommentLikesCounter
	postAuthor          PostAuthorGetter
	bus                 *events.TypedBus[events.CommentCreatedEvent]
	db                  *sql.DB
}

func NewServiceComment(commentStore CommentStore, postUpdater PostUpdater, cc CommentsCounter, clc CommentLikesCounter, postAuthor PostAuthorGetter, bus *events.TypedBus[events.CommentCreatedEvent], db *sql.DB) *CommentService {
	return &CommentService{commentStore: commentStore, postUpdater: postUpdater, commentsCounter: cc, commentLikesCounter: clc, postAuthor: postAuthor, bus: bus, db: db}
}
...
func (s *CommentService) GetAllByPostID(ctx context.Context, postID uuid.UUID, cursorStr string, limit int) (*PaginatedCommentResponse, error) {
	...
	s.applyLiveLikesCounts(ctx, comments)
...
func (s *CommentService) GetReplies(ctx context.Context, parentCommentID uuid.UUID) ([]*Comment, error) {
	replies, err := s.commentStore.GetRepliesByParentID(ctx, parentCommentID)
	if err != nil {
		return nil, err
	}
	s.applyLiveLikesCounts(ctx, replies)
	return replies, nil
}
...
func (s *CommentService) DeleteComment(ctx context.Context, userID, commentID uuid.UUID) error {
	...
	if _, err := s.commentsCounter.Decrement(ctx, c.PostID); err != nil {
		log.Printf("[comment] échec décrément comments_count Redis postID=%s: %v", c.PostID, err)
	}
...
func (s *CommentService) CreateComment(ctx context.Context, comment Comment) (*Comment, error) {
	...
	if _, err := s.commentsCounter.Increment(ctx, inserted.PostID); err != nil {
		log.Printf("[comment] échec incrément comments_count Redis postID=%s: %v", inserted.PostID, err)
	}

	authorID, err := s.postAuthor.GetAuthorID(ctx, inserted.PostID)
	if err != nil {
		log.Printf("[comment] échec récupération auteur postID=%s: %v", inserted.PostID, err)
		return inserted, nil
	}

	if authorID != inserted.UserID {
		s.bus.Publish(events.CommentCreatedEvent{
			CommentID:    inserted.CommentID,
			PostID:       inserted.PostID,
			PostAuthorID: authorID,
			ActorID:      inserted.UserID,
...
func (s *CommentService) applyLiveLikesCounts(ctx context.Context, comments []*Comment) {
	if len(comments) == 0 {
		return
	}
	...
	counts, err := s.commentLikesCounter.GetCounts(ctx, ids)
	if err != nil {
		log.Printf("[comment] échec lecture compteurs likes Redis: %v", err)
		return
	}
	for _, c := range comments {
		if n, ok := counts[c.CommentID]; ok {
			c.LikesCount = n
		}
	}
}
