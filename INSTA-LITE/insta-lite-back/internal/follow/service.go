...
type FollowService struct {
	...
	db            *sql.DB
	bus           *events.TypedBus[events.UserFollowedEvent]
}

func NewServiceFollow(followStore FollowStore, followUpdater FollowUpdater, db *sql.DB, bus *events.TypedBus[events.UserFollowedEvent]) *FollowService {
	return &FollowService{followStore: followStore, followUpdater: followUpdater, db: db, bus: bus}
}
...
func (s *FollowService) CreateFollow(ctx context.Context, followerID, followeeID uuid.UUID) (*Follow, error) {
	...
	if err != nil {
		...
		return nil, fmt.Errorf("%w: %v", ErrCreateFollowFailed, err)
	}
	...
	s.bus.Publish(events.UserFollowedEvent{
		FolloweeID: followeeID,
		ActorID:    followerID,
